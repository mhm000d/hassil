# SPEC-1-Hassil-Cash-Flow-Advance
...
- The system must support two complete account types: Small Business and Freelancer.
- The system must show an initial user-type selection page where the user chooses either Small Business or Freelancer.
- The system must provide separate onboarding forms for Small Business and Freelancer users.
- Small Business onboarding must collect business name, registration number, business email, phone number, country, and verified business bank account placeholder details.
- Freelancer onboarding must collect full name, email, phone number, country, and verified personal bank account placeholder details.
- The system must provide a dashboard for each user type.
- The dashboard must show total outstanding invoices, active advances, simulated available balance, trust score, and recent transactions.
- The system must allow users to create or upload invoices with invoice number, client name, client email, amount, currency, issue date, due date, description, payment terms, and receivable source.
- The system must support two receivable sources: direct client invoice and freelance platform payout.
- The system must map freelancers to the invoice discounting model by default.
- The system must map small businesses to the invoice factoring model by default.
- The system must allow both user types to request an advance against an invoice.
- The system must calculate the advance amount, flat fee, repayment amount, and settlement buffer before submission.
- The system must clearly show that the advance is not interest-based pricing and that the fee is fixed upfront.
- The system must require terms acceptance before submitting an advance request.
- The system must handle freelancer discounting without notifying the client.
- The system must handle small business factoring with client confirmation and payment redirection to Hassil.
- The system must include a lightweight invoice verification workflow using supporting document placeholders, duplicate-invoice checks, trust score, and optional client confirmation depending on model.
- The system must provide a client confirmation page for small business factoring where the client can confirm or dispute the invoice.
- The system must simulate approval or rejection based on financing model, invoice status, client confirmation when required, duplicate checks, user type, trust score, and admin decision.
- The system must simulate disbursement after approval using mocked Open Banking write responses.
- The system must show active advance details, including invoice amount, advance amount, fee, due date, financing model, repayment party, client, and repayment status.
- The system must simulate repayment differently for each model:
  - Freelancer discounting: client pays freelancer, then freelancer repays Hassil.
  - Small business factoring: client pays Hassil, then Hassil releases the remaining settlement buffer.
- The system must update trust score after successful repayment.
- The system must maintain a ledger-style transaction history for invoices, advances, repayments, fee collection, buffer release, and trust score changes.
- The system must include a demo-friendly admin reviewer page for pending and flagged advance requests.
- The system must be implemented as a web app only.
- The system must be English-only for all screens, labels, messages, and demo content.

### Should Have

- The system should use different limits, fees, and review rules for freelancer discounting and small business factoring.
- The system should allow small businesses to request larger advance amounts than freelancers.
- The system should prevent duplicate invoice submissions using invoice number, client email, amount, due date, and receivable source.
- The system should include pre-seeded demo users for one small business, one freelancer, one client, and one admin reviewer.
- The system should include pre-seeded demo data showing at least one completed repayment for each user type.
- The system should include an AI Review Assistant on the admin page that summarizes trust signals, risk flags, and a recommended action.
- The AI Review Assistant should not make the final approval decision. It should support the admin reviewer only.
- The system should use clean financial terminology that avoids loan, interest, debt-trap, or compounding language.
...
- The system could support a single “simulate next step” button for smoother hackathon demos.

### Won't Have in MVP

- The MVP will not move real money outside the sandbox simulation.
- The MVP will not integrate with real banks or payment gateways; it will use mocked Open Banking read/write responses.
- The MVP will not perform real KYC, KYB, AML, or credit bureau checks because users are assumed verified in the hackathon sandbox.
- The MVP will not provide a production-grade fraud detection or dispute-resolution engine.
- The MVP will not train a custom AI or machine learning model.
- The MVP will not let AI make final financing decisions.
- The MVP will not include mobile applications.
- The MVP will not support multiple languages.
- The MVP will not support real legal contracts or production regulatory workflows.
- The MVP will not integrate with external freelance platforms or accounting platforms.
...
3. Build authentication and demo login.
   ...
   - Seed admin user.
   - _Use simple JWT-style demo auth or mock it._
