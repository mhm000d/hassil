...
export default function NewInvoice() {
    ...
    const errors = [
        ...
        files.length === 0 ? 'Attach invoice evidence before submitting.' : '',
    ...
    const handleFileSelect = () => {
        ...
        setFiles([{ name: `invoice_${form.invoiceNumber}.pdf`, size: '1.2 MB' }])
    }
    ...
    const submit = async (e: FormEvent) => {
        ...
        // Formally add each "uploaded" file
        for (const file of files) {
            await addDocument(invoiceId, { fileName: file.name, documentType: 'InvoiceEvidence' })
        }
            ...
            <form className="card form-card" onSubmit={submit}>
                ...
                <div className="form-grid-2">
                    <div className="form-group">
                        ...
                        <input value={form.invoiceNumber} onChange={(e) => setForm({ ...form, invoiceNumber: e.target.value })} required disabled={isSubmitting} />
                    </div>
                    <div className="form-group">
                        <label>Receivable source</label>
                        <select value={form.receivableSource} onChange={(e) => setForm({ ...form, receivableSource: e.target.value as Invoice['receivableSource'] })} disabled={isSubmitting}>
                    ...
                    <div className="form-group">
                        ...
                        <input value={form.clientName} onChange={(e) => setForm({ ...form, clientName: e.target.value })} required disabled={isSubmitting} />
                    </div>
                    <div className="form-group">
                        <label>Client email</label>
                        <input type="email" value={form.clientEmail} onChange={(e) => setForm({ ...form, clientEmail: e.target.value })} required disabled={isSubmitting} />
                    </div>
                    <div className="form-group">
                        <label>Client country</label>
                        <input value={form.clientCountry} onChange={(e) => setForm({ ...form, clientCountry: e.target.value })} disabled={isSubmitting} />
                    </div>
                    <div className="form-group">
                        <label>Currency</label>
                        <select value={form.currency} onChange={(e) => setForm({ ...form, currency: e.target.value })} disabled={isSubmitting}>
                            <option>USD</option>
                            <option>AED</option>
                            <option>SAR</option>
                            <option>EGP</option>
                    ...
                    <div className="form-group">
                        <label>Amount</label>
                        <input type="number" min="1" value={form.amount} onChange={(e) => setForm({ ...form, amount: Number(e.target.value) })} required disabled={isSubmitting} />
                    </div>
                    <div className="form-group">
                        <label>Issue date</label>
                        <input type="date" value={form.issueDate} onChange={(e) => setForm({ ...form, issueDate: e.target.value })} required disabled={isSubmitting} />
                    </div>
                    <div className="form-group">
                        <label>Due date</label>
                        <input type="date" value={form.dueDate} onChange={(e) => setForm({ ...form, dueDate: e.target.value })} required disabled={isSubmitting} />
                    </div>
                    <div className="form-group">
                        <label>Payment terms</label>
                        <input value={form.paymentTerms} onChange={(e) => setForm({ ...form, paymentTerms: e.target.value })} disabled={isSubmitting} />
                    </div>
                </div>
                <div className="form-group">
                    <label>Description</label>
                    <textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} disabled={isSubmitting} />
                </div>
                
                <div className="form-group mt-12">
                    <label>Evidence & Documents</label>
                    <div className={`upload-zone ${files.length > 0 ? 'has-files' : ''}`} onClick={!isSubmitting ? handleFileSelect : undefined}>
                        {files.length === 0 ? (
                            <>
                                <Icon name="upload" />
                                <span>Click to upload invoice evidence (PDF, PNG)</span>
                            </>
                        ) : (
                            <div className="file-list">
                                {files.map(f => (
                                    <div key={f.name} className="file-pill">
                                        ...
                                        <button type="button" className="btn-icon" onClick={(e) => { e.stopPropagation(); setFiles([]); }}>×</button>
                                    </div>
                                ))}
                            </div>
                        )}
                ...
                {isSubmitting && (
                    <div className="upload-progress-bar mt-12">
                        <div className="progress-fill" style={{ width: `${uploadProgress}%` }} />
                        <span className="progress-text">Uploading... {uploadProgress}%</span>
                    </div>
                )}

                {(errors.length > 0 || warnings.length > 0) && !isSubmitting && (
                    <div className="feedback-list mb-18 mt-18">
                ...
                <div className="form-actions mt-24">
                    <button type="button" className="btn btn-ghost" onClick={() => navigate('/invoices')} disabled={isSubmitting}>Cancel</button>
                    <button className="btn btn-primary" type="submit" disabled={errors.length > 0 || isSubmitting}>
                        {isSubmitting ? 'Processing...' : <><Icon name="plus" /> Create & Submit Invoice</>}
                    </button>
                </div>
            </form>
        </>
